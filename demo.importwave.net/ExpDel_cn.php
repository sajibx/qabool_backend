...
$sql1 = "DELETE FROM `Daily3xp_cn` WHERE `sn` = ?";
...
$sqlY = "SELECT `sn`, `CashIH` from `Daily3xp_cn` WHERE `date` < ? ORDER BY `sn` DESC LIMIT 1;";
$CashIH = 0; // Initialize CashIH
...
if ($stmtY = $conn->prepare($sqlY)) {
    ...
    if ($rowY = $resultY->fetch_assoc()) {
        $CashIH = floatval($rowY['CashIH']); // Ensure it's treated as a number
    }
    $stmtY->close();
}

$sqlTRec = "SELECT SUM(`amount`) 'TREC' FROM `Daily3xp_cn` WHERE `date` = ? AND `type` = 'Rec'";
$TRec = 0;
...
$sqlTExp = "SELECT SUM(`amount`) 'TEXP' FROM `Daily3xp_cn` WHERE `date` = ? AND `type` = 'Exp'";
$TExp = 0;
...
$sql0 = "UPDATE `Daily3xp_cn`
            JOIN (
                SELECT MAX(`sn`) AS max_sn
                FROM `Daily3xp_cn`
            ) AS temp
            ON `Daily3xp_cn`.`sn` = temp.max_sn
            SET `CashIH` = ?;";
