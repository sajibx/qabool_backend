...
"columnDefs": [
    {
      className: "dtRight", targets: [ 4,7]
            ...
            "columns" : [
                // SELECT `SHIPMENT`, `CNFNAME`, `DATE`, `AWBNO`, `AMOUNT`, `TYPE`, `REMARKS`, `OUTSTANDING`
                {"title": "DATE", "data":"DATE"},
                {"title": "SHIPMENT", "data":"SHIPMENT"},
                {"title": "CNF NAME", "data":"CNFNAME"},
                {"title": "AWB NO", "data":"AWBNO"},
...
<div style="height: 35px; width: 100%; border: 0px solid red;">
            ...
            <div style="min-width: 100%; border:0px solid black; float: right; margin-bottom: 10px;">
                        ...
                        <span id="LedTitle" style="font-size: 26; color: 990000; letter-spacing: 5px; float: left; font-weight:bold ;">CNF LEDGER</span>
                                ...
                                <img onclick="GetPage('AdminLedger');" src='./images/reload.png' style='cursor: hand; width: 33px; float: right;
                                height: 30px;  border: 0px solid green;'>
                                
                        </div>

                <!--        <img title="New Entry" id="PlusIco" onclick="GetPage('CNFEntry');" src='./images/plus1.png' style='cursor: hand; width: 33px; float:left;
                            height: 30px;  border: 0px solid green; margin-left:30px;'>
...
<!-- -------------------------------------------------------------------------------------------- -->
<select class="customSelect classic" id="ClientName" onchange="GetTotalCNF();" style="width:200px; float:right; margin-right:30px;">
    <option style="text-align: center; font-weight: bolder; float:right; " value="CNF">CNF</option>
    <?php 
        include ("./conn.php");
        ...
        $stmt_item->execute();
        $result = $stmt_item->get_result();  // if mysqlnd isn't available, see fallback below
        $stmt_item->close();
...
<script type="text/javascript">
      function GetTotalCNF() {

        var ClientName = document.getElementById('ClientName').value;       

        var link = "./GetTotalCNF.php?FClient=" + ClientName;
