...
$LEDGERLIMIT   = $ini["GENERAL"]["LEDGERLIMIT"];
	...
	if($FClient == 'CLIENT'){
		...
		$sql = "SELECT  `DATE`, `sn`, `SHIPMENT`, `CLIENT`, `CHARG_WEIGHT`,`AMOUNT`, `TYPE`, `REMARKS`, `OUTSTANDING`

				FROM `billledger` ORDER BY `sn` DESC LIMIT $LEDGERLIMIT";
		...
		$sql = "SELECT  `DATE`, `sn`, `SHIPMENT`, `CLIENT`, `CHARG_WEIGHT`,`AMOUNT`, `TYPE`, `REMARKS`, `OUTSTANDING`
				...
				FROM `billledger` WHERE `CLIENT` = '$FClient' ORDER BY `sn` DESC";
